...
const Ive = iveMembers.map(
    (x) => new FemaleIdol(x.name, x.year)
);
console.log(Ive);

const BTS = btsMembers.map(
    (x) => new MaleIdol(x.name, x.year)
);
console.log(BTS);

const groupIve = new IdolGroup('아이브', Ive);
console.log(groupIve);

const groupBTS = new IdolGroup('BTS', BTS);
console.log(groupBTS);

const korea = new Country('Korea', [groupIve, groupBTS]);
console.log(korea);
